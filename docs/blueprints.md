...
لإنشاء كائن المُخطّط، نقوم باستيراد الصنف `()Blueprint` ونقوم بتهيئته بالمعاملات `name` و `import_name`. عادةً تُسنَد القيمة `__name__` للمعامل `import_name` وحسب تعد هذه القيمة متغيّر بايثون من النوع الخاص يحوي اسم الوحدة الحالية.
