...
```html
{# myapp/templates/layout.html ملف #}
...
ما قُمنا به في هذا المثال هو استداء ماكرو غير معرف (وهو الماكرو `nav_link`) وتمرير معاملين له: الأول اسم الدالة الهدف (أي اسم الدالة لطريقة العرض المستهدفة) والنص الذي نريد إظهاره.
...
<li>يمكنك إيجاد جميع المتغيرات العمومية (global) المُمرة إلى سياق جينجا تلقائياً من <a href='http://flask.pocoo.org/docs/templating/#standard-context'>هنا</a>.</li>
...
```html
{# myapp/templates/macros.html ملف #}
...
الآن قمنا بتعريف الماكرو في المسار *myapp/templates/macros.html*. في هذا الماكرو قمنا باستخدام الكائن `request` - والذي يكون متوفر إفتراضياً في سياق جينجا - للتأكد مما إذا كان الطلب الحالي قد وُجِهَ إلى الدالة المُمررة إلى الماكرو `nav_link`. إن كان، فبالتالي نحن في تلك الصفحة، ويمكننا تعليم رابطها كرابط نشط.
...
في المثال أعلاه، المرشح `title` سيأخذ `article.title` ويعيد النسخة المعنونة حرفياً (title cased) منه، والتي من ثم ستُطبَع إلى القالب. تشبه آلية عمل هذه الخاصية إلى حدٍ كبير آلية عمل "ضخ (piping)" مُخرج برنامج إلى مدخلات برنامج آخر في يونكس.
...
# تأكد أنه تم استهلال التطبيق أولاً لمنع مشاكل الاستيرادات الدائرية.
